  ...
  qx.Class.define("com.zenesis.qx.upload.AbstractHandler", {
    ...
    members: {
      ...
      _addFile: function _addFile(file) {
        if (this.__P_313_3.fireDataEvent("addFile", file, null, true)) this.__P_313_0.push(file);
      },
      ...
      beginUploads: function beginUploads() {
        while (this.__P_313_0.length > 0 && this.__P_313_1.length < this.getMaxConnections()) {
          var file = this.__P_313_0.shift();

          this.__P_313_1.push(file);

          this.__P_313_3.fireDataEvent("beginUpload", file);
      ...
      cancel: function cancel(file) {
        var wasUploading = this.__P_313_1.length > 0; // this.debug("cancelled: id=" + file.getId() + ", fileName=" +
        // file.getFilename());

        this._cancel(file);

        if (wasUploading && this.__P_313_3.getAutoUpload()) this.beginUploads();
      },
      ...
      cancelAll: function cancelAll() {
        for (var current = this.__P_313_1, i = 0; i < current.length; i++) {
          this._cancel(current[i]);
        }

        this.__P_313_1.splice(0, this.__P_313_1.length);

        this.__P_313_0.splice(0, this.__P_313_0.length);
      },
      ...
      _cancel: function _cancel(file) {
        ...
        for (var queue = this.__P_313_0, i = 0; i < queue.length; i++) {
          if (queue[i] == file) {
            queue.splice(i, 1);
            break;
          }
        ...
        this.__P_313_3.fireDataEvent("cancelUpload", file);
      },
      ...
      _onCompleted: function _onCompleted(file, response) {
        ...
        if (file.getState() == "uploading") {
          ...
          this.__P_313_3.fireDataEvent("completeUpload", file);
        } // Start the next one
      ...
      _getMergedParams: function _getMergedParams(file) {
        ...
        for (var name in this.__P_313_2) {
          var value = this.__P_313_2[name];
...
//# sourceMappingURL=AbstractHandler.js.map?dt=1604956089407