  ...
  qx.Mixin.define("qx.ui.tree.core.MWidgetController", {
    ...
    members: {
      ...
      bindDefaultProperties: function bindDefaultProperties(item, index) {
        ...
        var bindPath = this.__P_315_1(index);
      ...
      bindProperty: function bindProperty(sourcePath, targetProperty, options, targetWidget, index) {
        var bindPath = this.__P_315_1(index, sourcePath);

        var bindTarget = this._tree.getLookupTable();

        var id = bindTarget.bind(bindPath, targetWidget, targetProperty, options);

        this.__P_315_2(targetWidget, id);
      },
      ...
      bindPropertyReverse: function bindPropertyReverse(targetPath, sourceProperty, options, sourceWidget, index) {
        var bindPath = this.__P_315_1(index, targetPath);

        var bindTarget = this._tree.getLookupTable();

        var id = sourceWidget.bind(sourceProperty, bindTarget, bindPath, options);

        this.__P_315_2(sourceWidget, id);
      },
      ...
      removeBindings: function removeBindings() {
        while (this.__P_315_0.length > 0) {
          var item = this.__P_315_0.pop();
      ...
      _removeBindingsFrom: function _removeBindingsFrom(item) {
        var bindings = this.__P_315_3(item);
        ...
        if (this.__P_315_0.includes(item)) {
          qx.lang.Array.remove(this.__P_315_0, item);
        }
      },
       ...
       */
      __P_315_1: function __P_315_1(index, path) {
        var bindPath = "[" + index + "]";
       ...
       */
      __P_315_2: function __P_315_2(widget, id) {
        var bindings = this.__P_315_3(widget);

        if (!bindings.includes(id)) {
          bindings.push(id);
        }

        if (!this.__P_315_0.includes(widget)) {
          this.__P_315_0.push(widget);
        }
      },
       ...
       */
      __P_315_3: function __P_315_3(widget) {
...
//# sourceMappingURL=MWidgetController.js.map?dt=1619362544435