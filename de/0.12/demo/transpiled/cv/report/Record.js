  ...
  qx.Class.define('cv.report.Record', {
    ...
    */
    members: {
       ...
       */
      __P_63_10: function __P_63_10(nativeEvent) {
        var data = {
          ...
          "native": {
            ...
            currentTarget: nativeEvent.currentTarget ? this.__P_63_11(nativeEvent.currentTarget) : undefined,
            relatedTarget: nativeEvent.relatedTarget ? this.__P_63_11(nativeEvent.relatedTarget) : undefined,
      ...
      recordNativeEvent: function recordNativeEvent(ev) {
        ...
        ev.$$RID = this.__P_63_9;

        if (ev.type.endsWith("down") || ev.type.endsWith("start")) {
          this.__P_63_6 = this.__P_63_7;
        } else if (ev.type.endsWith("up") || ev.type.endsWith("end")) {
          this.__P_63_6 = this.__P_63_8;
        }

        if (/.+(move|over|out)/.test(ev.type)) {
          if (!this.__P_63_5[ev.type]) {
            this.__P_63_5[ev.type] = {
              x: ev.clientX,
              y: ev.clientY
            };
          } else {
            var lastDelta = this.__P_63_5[ev.type];

            if (Math.abs(lastDelta.x - ev.clientX) <= this.__P_63_6 || Math.abs(lastDelta.y - ev.clientY) <= this.__P_63_6) {
        ...
        var path = this.__P_63_11(ev.target);
      ...
      __P_63_11: function __P_63_11(el) {
        if (el === window) {
          return "Window";
        } else if (el === document) {
          return "document";
...
//# sourceMappingURL=Record.js.map?dt=1614551886783