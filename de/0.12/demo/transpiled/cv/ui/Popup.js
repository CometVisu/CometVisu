  ...
  qx.Class.define('cv.ui.Popup', {
    ...
    */
    construct: function construct(type) {
      ...
      this.__P_173_0 = ['#top', '#navbarTop', '#centerContainer', '#navbarBottom', '#bottom'];
      this.__P_173_1 = {};
    },
    ...
    */
    members: {
      ...
      create: function create(attributes) {
        ...
        if (!this.__P_173_3) {
          ret_val = this.__P_173_3 = qx.dom.Element.create("div", {
            id: "popup_" + this.__P_173_2,
            "class": classes.join(" "),
            style: "visibility:hidden",
            html: closable ? '<div class="popup_close">X</div>' : ""
          });
          body.appendChild(ret_val);
          this.__P_173_1.close = ret_val.querySelector("div.popup_close");
          addCloseListeners = true;
        } else {
          isNew = false;
          ret_val = this.__P_173_3;
          ret_val.setAttribute("class", classes.join(" "));

          if (closable && !this.__P_173_1.close) {
            this.__P_173_3.close = qx.dom.Element.create("div", {
              "class": "popup_close",
              "html": "X"
            });
            qx.dom.Element.insertBegin(this.__P_173_3.close, body);
            addCloseListeners = true;
          } else if (!closable) {
        ...
        this.__P_173_3.$$topic = attributes.topic;
        this.__P_173_3.$$page = attributes.page;

        if (attributes.title) {
          if (!this.__P_173_1.title) {
            this.__P_173_1.title = qx.dom.Element.create("div", {
              "class": "head"
            });
            ret_val.appendChild(this.__P_173_1.title);
          }

          if (qx.lang.Type.isString(attributes.title)) {
            this.__P_173_1.title.innerHTML = "" + attributes.title;
          } else {
            this.__P_173_1.title.appendChild(attributes.title);
          }
        }

        if (attributes.content || attributes.icon || attributes.progress) {
          if (!this.__P_173_1.content) {
            this.__P_173_1.content = qx.dom.Element.create("div", {
              "class": "main"
            });
            ret_val.appendChild(this.__P_173_1.content);
          }

          if (attributes.content) {
            if (!this.__P_173_1.messageContent) {
              this.__P_173_1.messageContent = qx.dom.Element.create("div", {
                "class": "message"
              });
              qx.dom.Element.insertBegin(this.__P_173_1.messageContent, this.__P_173_1.content);
            }

            if (qx.lang.Type.isString(attributes.content)) {
              this.__P_173_1.messageContent.innerHTML = attributes.content;
            } else {
              this.__P_173_1.messageContent.parentNode.replaceChild(attributes.content, this.__P_173_1.messageContent);

              this.__P_173_1.messageContent = attributes.content;
            }
          } else {
          ...
          if (attributes.icon) {
            if (!this.__P_173_1.icon) {
              var iconClasses = attributes.iconClasses ? " " + attributes.iconClasses : "";
              this.__P_173_1.icon = qx.dom.Element.create("div", {
                "html": cv.util.IconTools.svgKUF(attributes.icon)(null, null, "icon" + iconClasses)
              });
              qx.dom.Element.insertBegin(this.__P_173_1.icon, this.__P_173_1.content);
            } else {
              var use = this.__P_173_1.icon.querySelector("use");
          ...
          if (attributes.progress) {
            if (!this.__P_173_1.progress) {
              var bar = new cv.ui.util.ProgressBar();
              this.__P_173_1.progress = bar.getDomElement();

              this.__P_173_1.content.appendChild(this.__P_173_1.progress);
            }

            this.__P_173_1.progress.$$widget.setValue(attributes.progress);
          } else {
        ...
        if (attributes.actions && Object.getOwnPropertyNames(attributes.actions).length > 0) {
          if (!this.__P_173_1.actions) {
            this.__P_173_1.actions = qx.dom.Element.create("div", {
              "class": "actions"
            });
            ret_val.appendChild(this.__P_173_1.actions);
          } else {
            // clear content
            this.__P_173_1.actions.innerHTML = "";
          }
          ...
          Object.getOwnPropertyNames(attributes.actions).forEach(function (type, index) {
            ...
            var target = this.__P_173_1.actions;
            var wrapper = null;
      ...
      destroyElement: function destroyElement(name) {
        if (this.__P_173_1[name]) {
          this.__P_173_1[name].parentNode.removeChild(this.__P_173_1[name]);

          delete this.__P_173_1[name];
        }
      },
      ...
      close: function close() {
        if (this.__P_173_3) {
          cv.ui.BodyBlocker.getInstance().unblock(this.__P_173_3.$$topic);

          this.__P_173_3.parentNode.removeChild(this.__P_173_3);
...
//# sourceMappingURL=Popup.js.map?dt=1614107744343