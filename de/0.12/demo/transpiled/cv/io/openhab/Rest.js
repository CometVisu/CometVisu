    ...
    members: {
      ...
      getResourcePath: function getResourcePath(name, map) {
        if (name === 'charts' && map && map.src) {
          ...
          if (map.start) {
            var endTime = map.end ? this.__P_71_4(map.end) : new Date();
      ...
      __P_71_4: function __P_71_4(time) {
        if (time === "now") {
          return new Date();
        } else if (/^[\d]+$/.test(time)) {
          var d = new Date();
      ...
      authorize: function authorize(req) {
        if (this.__P_71_3) {
          req.setRequestHeader("Authorization", this.__P_71_3);
        }
      },
      ...
      __P_71_5: function __P_71_5(type, state) {
        switch (type) {
          case "Decimal":
          case "Percent":
          case "Number":
          ...
          res.forEach(function (entry) {
            ...
            if (entry.members && Array.isArray(entry.members)) {
              ...
              entry.members.forEach(function (obj) {
                ...
                if (_this.__P_71_5(obj.type, obj.state)) {
                  active++;
                }

                if (!_this.__P_71_1.hasOwnProperty(obj.name)) {
                  _this.__P_71_1[obj.name] = [entry.name];
                } else {
                  _this.__P_71_1[obj.name].push(entry.name);
                }

                return map;
              });
              this.__P_71_0[entry.name] = {
                members: map,
                active: active
              };
      ...
      handleMessage: function handleMessage(payload) {
        ...
        if (payload.type === "message") {
          ...
          if (data.type === "ItemStateChangedEvent" || data.type === "GroupItemStateChangedEvent") {
            ...
            if (this.__P_71_1.hasOwnProperty(item)) {
              var groupNames = this.__P_71_1[item];
              groupNames.forEach(function (groupName) {
                var group = _this2.__P_71_0[groupName];
                var active = 0;
                ...
                Object.keys(group.members).forEach(function (memberName) {
                  ...
                  if (_this2.__P_71_5(member.type, member.value)) {
                    active++;
                  }
                });
                group.active = active;
      ...
      login: function login(loginOnly, credentials, callback, context) {
        if (credentials && credentials.username) {
          ...
          this.__P_71_3 = "Basic " + btoa(credentials.username + ":" + (credentials.password || ""));
...
//# sourceMappingURL=Rest.js.map?dt=1620513276789