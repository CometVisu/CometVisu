...
.. list-table::
   ...
   * - KNX_INTERFACE
     - 
     - knxd-URL für die KNX Schnittstelle. Wenn leer, so wird der ``knxd`` nicht geladen.
   * - KNX_PA
     - ``1.1.238``
     - Physikalische Schnittstelle (PA) für den ``knxd``.
   * - KNX_CLIENT_PAS
     - ``1.1.239:10``
     - An Client-Verbindungen zu verteilender Adressbereich.
   * - KNX_INTERFACE_OPTIONS
     - ``-B single -A retry-delay=1``
     - Spezifische Aufruf-Parameter für die KNX Schnittstelle.
   * - CGI_URL_PATH
     - ``/cgi-bin/``
     - URL-Prefix um die ``cgi-bin`` Ressourcen zu lokalisieren.
   * - BACKEND_PROXY_SOURCE
     - 
     - Proxy-Pfade beginnen mit diesem Wert, z.B. ``/rest`` für das openHAB Backend.
   * - BACKEND_PROXY_TARGET
     - 
     - Ziel-URL für die Proxy-Anfragen an ``BACKEND_PROXY_SOURCE``, z.B. ``http://<openhab-server-ip-address>:8080/rest`` für das openHAB Backend.
