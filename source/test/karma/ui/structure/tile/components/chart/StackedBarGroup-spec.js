...
/**
 * Unit tests for cv.ui.structure.tile.components.chart.StackedBarGroup
 *
 * @author Tobias Bräutigam
 * @since 2025
 */
describe('testing the StackedBarGroup chart component', () => {
  let chart;
  let group;
  ...
  it('should create a StackedBarGroup', () => {
    group = new cv.ui.structure.tile.components.chart.StackedBarGroup(chart, 'stacked-bar');
    expect(group).not.toBeNull();
    expect(group.getType()).toBe('stacked-bar');
  });
  ...
  it('should initialize with chart', () => {
    group = new cv.ui.structure.tile.components.chart.StackedBarGroup(chart, 'stacked-bar');
    expect(chart.getSvgElement).toHaveBeenCalledWith('g', ['stacked-bar'], jasmine.any(Object));
  });
});
