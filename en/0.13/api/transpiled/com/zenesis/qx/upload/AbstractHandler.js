  ...
  qx.Class.define("com.zenesis.qx.upload.AbstractHandler", {
    ...
    members: {
      ...
      _addFile: function _addFile(file) {
        if (this.__P_820_3.fireDataEvent("addFile", file, null, true)) this.__P_820_0.push(file);
      },
      /**
       * Begins spooling uploads to the server, up to the maxConnections
       */
      beginUploads: function beginUploads() {
        while (this.__P_820_0.length > 0 && this.__P_820_1.length < this.getMaxConnections()) {
          var file = this.__P_820_0.shift();
          this.__P_820_1.push(file);
          this.__P_820_3.fireDataEvent("beginUpload", file);
      ...
      cancel: function cancel(file) {
        var wasUploading = this.__P_820_1.length > 0;
        // this.debug("cancelled: id=" + file.getId() + ", fileName=" +
        // file.getFilename());
        this._cancel(file);
        if (wasUploading && this.__P_820_3.getAutoUpload()) this.beginUploads();
      },
      /**
       * Cancels all uploads
       */
      cancelAll: function cancelAll() {
        for (var current = this.__P_820_1, i = 0; i < current.length; i++) this._cancel(current[i]);
        this.__P_820_1.splice(0, this.__P_820_1.length);
        this.__P_820_0.splice(0, this.__P_820_0.length);
      },
      /**
       * Cancels a file
       *
      ...
      _cancel: function _cancel(file) {
        var inCurrent = false;
        for (var current = this.__P_820_1, i = 0; i < current.length; i++) if (current[i] == file) {
          current.splice(i, 1);
          inCurrent = true;
          break;
        }
        for (var queue = this.__P_820_0, i = 0; i < queue.length; i++) if (queue[i] == file) {
          queue.splice(i, 1);
          break;
        }
        file.setState("cancelled");
        if (inCurrent) this._doCancel(file);
        this.__P_820_3.fireDataEvent("cancelUpload", file);
      },
      /**
      ...
      _onCompleted: function _onCompleted(file, response) {
        ...
        if (file.getState() == "uploading") {
          ...
          this.__P_820_3.fireDataEvent("completeUpload", file);
        }
      ...
      _getMergedParams: function _getMergedParams(file) {
        var result = {};
        for (var name in this.__P_820_2) {
          var value = this.__P_820_2[name];
...
//# sourceMappingURL=AbstractHandler.js.map?dt=1782595085425