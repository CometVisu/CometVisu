    ...
    members: {
            ...
            while (1) {
              switch (_context.prev = _context.next) {
                case 0:
                  ...
                  _this.parseFiles(xml); // parse the icons


                  xml.querySelectorAll('meta > icons icon-definition').forEach(_this.parseIcons, _this); // then the mappings

                  xml.querySelectorAll('meta > mappings mapping').forEach(_this.parseMappings, _this); // then the stylings

                  xml.querySelectorAll('meta > stylings styling').forEach(_this.parseStylings, _this); // then the status bar
      ...
      parseStateNotifications: function parseStateNotifications(xml) {
        ...
        xml.querySelectorAll('meta > notifications state-notification').forEach(function (elem) {
          ...
          var addresses = cv.parser.pure.WidgetParser.makeAddressList(addressContainer); // addresses
                      ...
                      templates.forEach(function (elem) {
                        ...
                        if (ref) {
                          ...
                          areq.addListenerOnce('success', function (e) {
                            var req = e.getTarget();
                            cv.parser.pure.WidgetParser.addTemplate(templateName, // templates can only have one single root element, so we wrap it here
...
//# sourceMappingURL=MetaParser.js.map?dt=1664548958467