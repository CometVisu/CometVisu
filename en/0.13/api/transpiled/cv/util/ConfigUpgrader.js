    ...
    members: {
      ...
      upgrade: function upgrade(source) {
        ...
        if (version === cv.Version.LIBRARY_VERSION_PURE) {
          // nothing to do
          return [null, source, this.__P_771_1];
        }
        ...
        while (version < systemLibVersion) {
          ...
          if (method) {
            ...
            return [qx.locale.Manager.tr('Upgrader from version %1 not implemented', version), source, this.__P_771_1];
          }
        }
        this.info('  - ' + this.__P_771_1.join('\n  - '));
        return [null, source, this.__P_771_1];
      },
      from7to8Pure: function from7to8Pure(source) {
        ...
        this.__P_771_2(source, 8);
        if (c > 0) {
          this.__P_771_1.push('removed ' + c + ' \'plugin\'-nodes with obsolete plugin (gweather)');
        }
        return 8;
      },
      from8to9Pure: function from8to9Pure(source) {
        var _this = this;
        var c = 0;
        var singleIndent = ''.padEnd(this.__P_771_0, ' ');
        source.querySelectorAll('multitrigger').forEach(function (node) {
          var level = _this.__P_771_3(node);
          level++;
          var indent = ''.padEnd(_this.__P_771_0 * level, ' ');
          var buttonConf = {};
          ...
          if (buttonIds.length > 0) {
            ...
            node.appendChild(source.createTextNode('\n' + ''.padEnd(_this.__P_771_0 * (level - 1), ' ')));
            c++;
          }
        });
        if (c > 0) {
          this.__P_771_1.push('converted ' + c + ' \'multitrigger\'-nodes to new button configuration');
        }
        c = 0;
        ...
        if (c > 0) {
          this.__P_771_1.push('removed ' + c + ' \'plugin\'-nodes with obsolete plugin (colorchooser)');
        }
        this.__P_771_2(source, 9);
        return 9;
      },
      __P_771_3: function __P_771_3(node) {
        var level = 1;
        var parent = node.parentNode;
      ...
      __P_771_2: function __P_771_2(xml, version, isTile) {
        if (isTile === true) {
...
//# sourceMappingURL=ConfigUpgrader.js.map?dt=1735222453550