  ...
  qx.Class.define('cv.data.Simulation', {
    ...
    */
    construct: function construct(configFile, client) {
      ...
      this._client = client; // override PHP support check, because the responses are faked
    ...
    */
    members: {
      ...
      __P_557_4: function __P_557_4() {
        ...
        this.__P_557_5 = cv.Config.initialDemoData.xhr; // we need to adjust the timestamps of the chart data
        ...
        sinon.sandbox = sinon; // override sinons filter handling to be able to manipulate the target URL from the filter
      ...
      prepareTestData: function prepareTestData(addresses) {
        ...
        if (cv.Config.initialDemoData && addresses.length > 0) {
          if (cv.Config.initialDemoData.encodeFirst) {
            this.__P_557_1 = true; // connect address data from widgets first
            ...
            try {
              for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
                ...
                for (var _ga4 in seq.data) {
                  if (_ga4 in this._addressConfigs) {
                    seq.data[_ga4] = cv.Transform.encodeBusAndRaw(this._addressConfigs[_ga4], seq.data[_ga4]).raw;
                  }
                }
              }
            } catch (err) {
                  ...
                  return valueArray.map(function (val) {
                    if (val.indexOf('|') >= 0) {
                      var _val$split3 = val.split('|'),
                          ...
                          startAddress = _val$split4[0],
                          startVal = _val$split4[1];
            ...
            for (var _ga2 in cv.Config.initialDemoData.simulations) {
              _loop2(_ga2);
            }
          }
...
//# sourceMappingURL=Simulation.js.map?dt=1685978161819