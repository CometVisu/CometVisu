    ...
    members: {
      ...
      */
            ...
            while (1) switch (_context2.prev = _context2.next) {
              ...
              case 5:
                ...
                if (seriesSelection.length > 0) {
                  ...
                  title.appendChild(i); // current selection
                  ...
                  title.appendChild(popup); // make title clickable
                  ...
                  button.setAttribute('title', qx.locale.Manager.tr('next')); // initially we cannot go into the future
                ...
                if (!inBackground && element.hasAttribute('allow-fullscreen') && element.getAttribute('allow-fullscreen') === 'true') {
                  ...
                  _this.appendToHeader(_button, 'right'); // address
                  ...
                  element.parentElement.appendChild(tileAddress); // listen to parent tile of popup is opened or not
                  ...
                  if (parent) {
                    ...
                    parent.addEventListener('stateUpdate', function (ev) {
                      tileWidget.onStateUpdate(ev); // cancel event here
                ...
                _this.__P_75_9(); // check if we have a read address for live updates
      ...
      _onSeriesChange: function _onSeriesChange(select) {
        this._initializing = true; // reset offset when series changed

        this.resetCurrentPeriod();
        this._initializing = false; // reset configuration, we need a new one
      ...
      _loadData: function _loadData() {
        ...
        var _loop = function _loop() {
          url = client.getResourcePath('charts', {
            src: src,
            start: _this2.getStartTime(),
            end: _this2.getEndTime()
          });
            ...
            if (tsdata !== null) {
              ...
              try {
                for (_iterator4.s(); !(_step4 = _iterator4.n()).done;) {
                  var _step4$value2 = _slicedToArray(_step4.value, 2),
                      time = _step4$value2[0],
                      value = _step4$value2[1];
      ...
      _onRendered: function _onRendered(chartData, retries) {
        ...
        if (this.isVisible()) {
          var _this$_getSize3 = this._getSize(),
              _this$_getSize4 = _slicedToArray(_this$_getSize3, 2),
              width = _this$_getSize4[0],
              height = _this$_getSize4[1];
      ...
      _renderChart: function _renderChart(data, single) {
        ...
        var svg = d3.select(this._element).select('svg'); // Compute values.
        ...
        if (single) {
          ...
          if (X[X.length - 1] === data.time) {
            ...
            Z.push(data.src);
            O.push(data);
            T.push(config.title === undefined ? data.src : config.title === null ? null : config.title(data)); // cleanup
        ...
        var zDomain = new d3.InternSet(Z); // Omit any data not present in the z-domain.
        ...
        if (config.showYAxis) {
          var maxValue = config.yFormat ? config.yFormat(yDomain[1]) : yDomain[1].toFixed(maxVal < 1 ? 2 : 1); // check if we need more space for the y-axis
        ...
        if (!this._chartConf) {
          ...
          this._chartConf.yAxis = config.showYAxis ? d3.axisLeft(this._chartConf.y).ticks(yTicks, config.yFormat) : undefined; // add elements
        ...
        var _iterator10 = _createForOfIteratorHelper(this._element.querySelectorAll(':scope > h-line').entries()),
            _step10;
        ...
        try {
          var _loop7 = function _loop7() {
            ...
            if (isNaN(yValue) && !lineElem.empty()) {
              ...
              var x1 = _this5._chartConf.x(X[data[0]]);

              var x2 = _this5._chartConf.x(X[X.length - 1]); // always draw until end of chart (not until end of src dataset)


              var y = _this5._chartConf.y(yValue);
              ...
              if (line.getAttribute('show-value') === 'true') {
                ...
                valueElem.attr('x', x2 + 2).attr('y', y + 5).text(cv.util.String.sprintf(format, yValue));
              }
            }
          };
      ...
      _onPointerMoved: function _onPointerMoved(event, center) {
        if (this._loaded && this.__P_75_2) {
          ...
          var _this$_helpers2 = this._helpers,
              X = _this$_helpers2.X,
              Y = _this$_helpers2.Y,
              I = _this$_helpers2.I,
              T = _this$_helpers2.T,
              Z = _this$_helpers2.Z,
              O = _this$_helpers2.O;
          var _this$_chartConf2 = this._chartConf,
              x = _this$_chartConf2.x,
              y = _this$_chartConf2.y,
              xz = _this$_chartConf2.xz;
          ...
          var scaleFactorY = this._element.offsetHeight / this.__P_75_8.height; // closest point
      ...
      getWeekNumber: function getWeekNumber(d) {
        d = new Date(d.getFullYear(), d.getMonth(), d.getDate()); // Set to the nearest Thursday: current date + 4 - current day number

        d.setDate(d.getDate() + 4 - (d.getDay() || 7)); // Get first day of year

        var yearStart = new Date(d.getFullYear(), 0, 1); // Calculate full weeks to the nearest Thursday
...
//# sourceMappingURL=Chart.js.map?dt=1685978102350