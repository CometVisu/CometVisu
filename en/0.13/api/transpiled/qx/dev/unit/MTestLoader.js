    ...
    members: {
      ...
      runStandAlone: function runStandAlone() {
        ...
        testResult.addListener("failure", function (e) {
          ...
          _this.error("Test '" + test.getFullName() + "' failed: " + ex.message + " - " + ex.getComment());
          if (ex.getStackTrace) {
            _this.error("Stack trace: " + ex.getStackTrace().join("\n"));
          }
        });
        testResult.addListener("error", function (e) {
          var ex = e.getData()[0].exception;
          var test = e.getData()[0].test;
          _this.error("The test '" + test.getFullName() + "' had an error: " + ex, ex);
        });
        this.getSuite().run(testResult);
      },
      /**
...
//# sourceMappingURL=MTestLoader.js.map?dt=1735341768036