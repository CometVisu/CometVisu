  ...
  ************************************************************************ */
  ...
  qx.Class.define("qx.ui.progressive.renderer.table.Row", {
    ...
    construct: function construct(columnWidths) {
      ...
      this.__P_422_6 = new qx.ui.layout.HBox();
      this.__P_422_6.connectToWidget(this);

      // dynamic theme switch
      {
        qx.theme.manager.Meta.getInstance().addListener("changeTheme", this.__P_422_5, this);
      }
    },
    statics: {
      /**
    ...
    members: {
       ...
       */
      __P_422_5: function __P_422_5() {
        // link to color theme
        var colorMgr = qx.theme.manager.Color.getInstance();
        this.__P_422_4.bgcol = [];
        this.__P_422_4.bgcol[0] = colorMgr.resolve("progressive-table-row-background-even");
        this.__P_422_4.bgcol[1] = colorMgr.resolve("progressive-table-row-background-odd");
      },
      // overridden
      join: function join(progressive, name) {
        ...
        this.__P_422_3 = progressive;

        // Save the name that Progressive knows us by
        this.__P_422_10 = name;
        ...
        if (!tr.__P_422_7[hash]) {
          ...
          tr.__P_422_7[hash].rowstylesheet = qx.bom.Stylesheet.createElement(stylesheet);
          var columnData = this.__P_422_0.getData();
          for (var i = 0; i < columnData.length; i++) {
            var stylesheet = ".qx-progressive-" + hash + "-col-" + i + " {" + tr.__P_422_9 + "}";
            tr.__P_422_7[hash].cellstylesheet[i] = qx.bom.Stylesheet.createElement(stylesheet);
          }

          // Save the hash too
          this.__P_422_11 = hash;
      ...
      addRenderer: function addRenderer(column, renderer) {
        ...
        this.__P_422_1[column] = renderer;
      },
      /**
      ...
      render: function render(state, element) {
        ...
        if (state.getRendererData()[this.__P_422_10].end === undefined) {
          state.getRendererData()[this.__P_422_10] = {
            end: 0,
            start: 1,
            rows: 0,
            totalHeight: 0
        ...
        for (var i = 0; i < data.length; i++) {
          var stylesheet = "qx-progressive-" + this.__P_422_11 + "-col-" + i;

          // Determine what renderer to use for this column
          renderer = this.__P_422_1[i] || this.__P_422_2;
        ...
        var rendererData = state.getRendererData()[this.__P_422_10];
        ...
        div.className = "qx-progressive-" + this.__P_422_11 + "-row";
        ...
        switch (element.location) {
          case "end":
            ...
            div.style.backgroundColor = this.__P_422_4.bgcol[index];
      ...
      _resizeColumns: function _resizeColumns(e) {
        var pane = this.__P_422_3.getStructure().getPane();
        var width = pane.getBounds().width - qx.bom.element.Scroll.getScrollbarWidth();

        // Get the style sheet rule name for this row
        var stylesheet = ".qx-progressive-" + this.__P_422_11 + "-row";

        // Remove the style rule for this row
        var tr = qx.ui.progressive.renderer.table.Row;
        qx.bom.Stylesheet.removeRule(tr.__P_422_7[this.__P_422_11].rowstylesheet, stylesheet);

        // Create the new rule for this row
        var rule = "width: " + width + "px;";

        // Apply the new rule
        qx.bom.Stylesheet.addRule(tr.__P_422_7[this.__P_422_11].rowstylesheet, stylesheet, rule);

        // Compute the column widths
        this.__P_422_6.renderLayout(width, 100, {
          top: 0,
          right: 0,
          bottom: 0,
          left: 0
        ...
        for (var i = 0, left = 0; i < columnData.length; i++, left += width) {
          ...
          var tr = qx.ui.progressive.renderer.table.Row;
          qx.bom.Stylesheet.removeRule(tr.__P_422_7[this.__P_422_11].cellstylesheet[i], stylesheet);
          ...
          if (qx.core.Environment.get("css.boxmodel") == "content") {
            inset = qx.ui.progressive.renderer.table.Row.__P_422_8 * 2;
          } else {
            inset = -1;
          }
          ...
          var paddingRule = "0px " + qx.ui.progressive.renderer.table.Row.__P_422_8 + "px " + "0px " + qx.ui.progressive.renderer.table.Row.__P_422_8 + "px;";
          var leftRule = left + "px;";
          var rule = tr.__P_422_9 + "width: " + widthRule + "left: " + leftRule + "padding: " + paddingRule;

          // Apply the new rule
          qx.bom.Stylesheet.addRule(tr.__P_422_7[this.__P_422_11].cellstylesheet[i], stylesheet, rule);
        }
      }
    },
    destruct: function destruct() {
      // remove dynamic theme listener
      qx.theme.manager.Meta.getInstance().removeListener("changeTheme", this.__P_422_5, this);
      var name;
      for (name in this.__P_422_1) {
        this.__P_422_1[name] = null;
      }

      // Remove any style sheets that we had added
      var tr = qx.ui.progressive.renderer.table.Row;
      var hash = this.__P_422_3.toHashCode();
      if (tr.__P_422_7 && tr.__P_422_7[hash]) {
        // Remove the row stylesheet
        if (tr.__P_422_7[hash].rowstylesheet) {
          // Get the style sheet rule name for this row
          var stylesheet = ".qx-progressive-" + this.__P_422_11 + "-row";
        ...
        if (tr.__P_422_7[hash].cellstylesheet) {
          for (var i = tr.__P_422_7[hash].cellstylesheet.length - 1; i >= 0; i--) {
            // Get the style sheet rule name for this cell
            var stylesheet = ".qx-progressive-" + this.__P_422_11 + "-col-" + i;
            var rule = tr.__P_422_7[this.__P_422_11].cellstylesheet[i];
      ...
      if (this.__P_422_3 && this.__P_422_3.getRendererData) {
        var rendererData = this.__P_422_3.getRendererData();
        if (rendererData && rendererData[this.__P_422_10] && rendererData[this.__P_422_10].end !== undefined) {
          rendererData[this.__P_422_10] = null;
        }
      }
      this.__P_422_4 = this.__P_422_1 = this.__P_422_3 = this.__P_422_0 = null;
      this._disposeObjects("__P_422_6", "__P_422_2", "__columnData");
    }
  });
  qx.ui.progressive.renderer.table.Row.$$dbClassInfo = $$dbClassInfo;
})();

//# sourceMappingURL=Row.js.map?dt=1673093873365