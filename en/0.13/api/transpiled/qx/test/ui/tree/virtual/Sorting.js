  ...
  qx.Class.define("qx.test.ui.tree.virtual.Sorting", {
    ...
    members: {
      testSorter: function testSorter() {
        ...
        this.__P_447_0(sortedModel, sorter);
        var delegate = {
          sorter: sorter
        };
      ...
      testModelUnmodified: function testModelUnmodified() {
        ...
        this.tree.setDelegate(delegate);
        this.flush();
        this.__P_447_2(rootChildrenClone.toArray(), root.getChildren().toArray());
      ...
      __P_447_0: function __P_447_0(model, sorter) {
        var children = model.getChildren();
        children.sort(sorter);
        for (var i = 0; i < children.getLength(); i++) {
          var child = children.getItem(i);
          if (child instanceof qx.test.ui.tree.virtual.Node) {
            this.__P_447_0(child, sorter);
          }
        }
      },
      __P_447_1: function __P_447_1(expected) {
      ...
      __P_447_2: function __P_447_2(expected, found) {
        var msg = "Expected [" + expected.join(", ") + "], but found [" + found.join(", ") + "]";
        this.assertEquals(expected.length, found.length, msg);
        for (var i = 0; i < expected.length; i++) {
          this.assertTrue(expected[i].equals(found[i]), msg);
      ...
      ---------------------------------------------------------------------------
      */
      __P_447_3: function __P_447_3(model, level) {
        ...
        var children = model.getChildren();
        for (var i = 0; i < children.getLength(); i++) {
          this.__P_447_3(children.getItem(i), level + 1);
        }
      }
    }
  });
...
//# sourceMappingURL=Sorting.js.map?dt=1782595067295