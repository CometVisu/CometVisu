  ...
  ************************************************************************ */
  ...
  qx.Bootstrap.define("qx.bom.element.AnimationJs", {
    statics: {
       ...
       */
      __P_141_1: ["%", "in", "cm", "mm", "em", "ex", "pt", "pc", "px"],
      ...
      _animate: function _animate(el, desc, duration, reverse) {
        ...
        var keys = this.__P_141_3(keyFrames);
        var stepTime = this.__P_141_4(duration, keys);
        var steps = parseInt(duration / stepTime, 10);
        this.__P_141_5(keyFrames, el);
        var delta = this.__P_141_6(steps, stepTime, keys, keyFrames, duration, desc.timing);
        ...
        handle.i = 0;
        handle.initValues = {};
        handle.repeatSteps = this.__P_141_7(steps, desc.repeat);
       ...
       * @param el {Element} The element to animate.
       */
      __P_141_5: function __P_141_5(keyFrames, el) {
          ...
          for (var name in keyFrames[percent]) {
            ...
            if (units[name] == undefined) {
              ...
              if (typeof item == "string") {
                units[name] = this.__P_141_8(item);
              } else {
                units[name] = "";
              }
            }
        ...
        for (var percent in keyFrames) {
          ...
          for (var name in units) {
            if (frame[name] == undefined) {
              ...
              if (frame[name] === "" && this.__P_141_1.indexOf(units[name]) != -1) {
       ...
       */
      __P_141_9: function __P_141_9(frame) {
        frame = qx.lang.Object.clone(frame);
        var transforms;
        for (var name in frame) {
          if (name in this.__P_141_2) {
            if (!transforms) {
              transforms = {};
            }
       ...
       */
      __P_141_6: function __P_141_6(steps, stepTime, keys, keyFrames, duration, timing) {
        var delta = new Array(steps);
        var keyIndex = 1;
        delta[0] = this.__P_141_9(keyFrames[0]);
            ...
            if (name in this.__P_141_2) {
              ...
              if (qx.Bootstrap.isArray(last[name])) {
                ...
                for (var j = 0; j < next[name].length; j++) {
                  ...
                  transforms[name][j] = this.__P_141_10(item, last[name], timing, x);
                }
              } else {
                var x = calculationIndex / stepsToNext;
                transforms[name] = this.__P_141_10(nItem, last[name], timing, x);
              }
              ...
              var x = calculationIndex / stepsToNext;
              delta[i][name] = this.__P_141_10(nItem, last[name], timing, x);
            } else {
        ...
        delta[delta.length - 1] = this.__P_141_9(keyFrames[100]);
        return delta;
      },
      /**
       ...
       */
      __P_141_8: function __P_141_8(item) {
       ...
       */
      __P_141_10: function __P_141_10(nextItem, lastItem, timing, x) {
        var range = parseFloat(nextItem) - parseFloat(lastItem);
        return parseFloat(lastItem) + range * qx.bom.AnimationFrame.calculateTiming(timing, x) + this.__P_141_8(nextItem);
      },
      /**
      ...
      play: function play(handle) {
        ...
        var id = window.setInterval(function () {
          ...
          qx.bom.element.AnimationJs.__P_141_11(handle.el, values);
          handle.i++;
          // iteration condition
      ...
      stop: function stop(handle) {
        ...
        if (keep != undefined && !handle.stopped) {
          ...
          this.__P_141_11(el, this.__P_141_9(desc.keyFrames[keep]));
        } else {
          this.__P_141_11(el, initValues);
        }
        el.$$animation = null;
        handle.el = null;
        handle.ended = true;
       ...
       */
      __P_141_7: function __P_141_7(steps, repeat) {
        if (repeat == undefined) {
          return steps;
        }
        if (repeat == "infinite") {
       ...
       */
      __P_141_11: function __P_141_11(el, styles) {
       ...
       */
      __P_141_4: function __P_141_4(duration, keys) {
        // get min difference
        var minDiff = 100;
       ...
       */
      __P_141_3: function __P_141_3(keyFrames) {
...
//# sourceMappingURL=AnimationJs.js.map?dt=1735341765555