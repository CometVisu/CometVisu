    ...
    members: {
      ...
      */
      ...
      dispatchEvent: function dispatchEvent(target, event, type) {
        ...
        var self = this;
        var tracker = {};
        var __P_215_0 = false; //(event._type == "pointerup" && event._target.className === "qx-toolbar-button-checked");
        var __P_215_1 = function __P_215_1() {};
        if (__P_215_0) {
          var serial = (this.SERIAL || 0) + 1;
          this.SERIAL = serial + 1;
          __P_215_1 = function __P_215_1() {
        ...
        qx.event.Utils["catch"](tracker, function () {
          ...
          __P_215_1("Aborted serial=" + serial + ", type=" + event.getType());
        });
        ...
        qx.event.Utils.then(tracker, function () {
          ...
          __P_215_1("captureList=" + captureList.length);
          return qx.event.Utils.series(captureList, function (localList, i) {
            __P_215_1("captureList[" + i + "]: localList.length=" + localList.length);
            ...
            var result = qx.event.Utils.series(localList, function (listener, listenerIndex) {
              ...
              if (!self._manager.isBlacklisted(listener.unique)) {
                __P_215_1("captureList[" + i + "] => localList[" + listenerIndex + "] callListener");
                return listener.handler.call(context, event);
              } else {
                __P_215_1("captureList[" + i + "] => localList[" + listenerIndex + "] is blacklisted");
              }
            }, true);
        ...
        qx.event.Utils.then(tracker, function () {
          ...
          __P_215_1("targetList=" + targetList.length);
          return qx.event.Utils.series(targetList, function (localList, i) {
            __P_215_1("targetList[" + i + "] localList.length=" + localList.length);
            var result = qx.event.Utils.series(localList, function (listener, listenerIndex) {
              __P_215_1("targetList[" + i + "] -> localList[" + listenerIndex + "] callListener");
              context = listener.context || target;
              __P_215_1("Calling target serial=" + serial + ", type=" + event.getType());
              return listener.handler.call(context, event);
            }, true);
            if (result === qx.event.Utils.ABORT) {
              return qx.event.Utils.reject(tracker);
        ...
        qx.event.Utils.then(tracker, function () {
          ...
          __P_215_1("bubbleList=" + bubbleList.length);
          return qx.event.Utils.series(bubbleList, function (localList, i) {
            __P_215_1("bubbleList[" + i + "] localList.length=" + localList.length);
            var currentTarget = bubbleTargets[i];
            event.setCurrentTarget(currentTarget);
            var result = qx.event.Utils.series(localList, function (listener, listenerIndex) {
              __P_215_1("bubbleList[" + i + "] -> localList[" + listenerIndex + "] callListener");
              context = listener.context || currentTarget;
              return listener.handler.call(context, event);
            }, true);
            if (result === qx.event.Utils.ABORT) {
        ...
        if (__P_215_0) {
          if (tracker.promise) {
            __P_215_1("events promised");
            qx.event.Utils.then(tracker, function () {
              __P_215_1("events promised done");
            });
          } else {
            __P_215_1("events done");
          }
        }
...
//# sourceMappingURL=AbstractBubbling.js.map?dt=1735383853673