  ...
  ************************************************************************ */
  ...
  qx.Class.define("qx.data.controller.Tree", {
    ...
    */
     ...
     */
    construct: function construct(model, target, childPath, labelPath) {
      qx.core.Object.constructor.call(this); // internal bindings reference

      this.__P_174_0 = {};
      this.__P_174_1 = []; // reference to the child
    ...
    */
    members: {
      ...
      */
      ...
      _applyChildPath: function _applyChildPath(value, old) {
        ...
        this.__P_174_6(); // reset the old name
      ...
      */
       ...
       */
      __P_174_7: function __P_174_7(ev) {
        // get the stored data
        var children = ev.getTarget();
        qx.core.ObjectRegistry.register(children);

        var treeNode = this.__P_174_2[children.toHashCode()].treeNode;

        var modelNode = this.__P_174_2[children.toHashCode()].modelNode; // update the subtree


        this.__P_174_8(treeNode, modelNode); // update the selection in case a selected element has been removed
       ...
       */
      __P_174_9: function __P_174_9(e) {
        var children = e.getData();
        var oldChildren = e.getOldData(); // get the old ref and delete it

        var oldRef = this.__P_174_2[oldChildren.toHashCode()];
        ...
        delete this.__P_174_2[oldChildren.toHashCode()]; // remove the old change listener for the children

        oldRef.modelNode.removeListenerById(oldRef.changeChildernListenerId); // add a new change listener
        ...
        var changeChildernListenerId = modelNode.addListener(eventName, this.__P_174_9, this); // add the new ref

        var treeNode = oldRef.treeNode;
        this.debug("1: adding children=" + children.toHashCode() + " to this=" + this.toHashCode());
        this.__P_174_2[children.toHashCode()] = {
          modelNode: modelNode,
          treeNode: treeNode,
      ...
      */
      ...
      __P_174_6: function __P_174_6() {
        ...
        this.__P_174_5(); // only build up a new tree if a model is given
        ...
        if (this.getModel() != null) {
          ...
          rootNode.setModel(this.getModel()); // bind the root node

          this.__P_174_10(this.getModel(), rootNode);

          this.__P_174_8(rootNode, this.getModel()); // assign the new root once the tree has been built
       ...
       */
      __P_174_8: function __P_174_8(rootNode, modelNode) {
        ...
        var children = modelNode["get" + qx.lang.String.firstUp(this.getChildPath())](); // store the children reference

        if (this.__P_174_2[children.toHashCode()] == undefined) {
          // add the listener for the change
          var changeListenerId = children.addListener("change", this.__P_174_7, this); // add a listener for the change of the children array itself

          var property = qx.Class.getPropertyDefinition(modelNode.constructor, this.getChildPath());
          var eventName = property.event;
          var changeChildernListenerId = modelNode.addListener(eventName, this.__P_174_9, this);
        ...
        for (var i = 0; i < children.length; i++) {
          ...
          if (rootNode.getChildren()[i] == null || children.getItem(i) != rootNode.getChildren()[i].getModel()) {
            ...
            if (oldIndex != undefined) {
              ...
              var currentNode = rootNode.getChildren()[oldIndex]; // check if it is selected
              ...
              rootNode.removeAt(oldIndex); // add the node at the current position

              rootNode.addAt(currentNode, i); // select it again if it was selected
              ...
              this.__P_174_10(children.getItem(i), treeNode); // add all children recursive


              this.__P_174_8(treeNode, children.getItem(i));
            }
          }
        ...
        for (var i = rootNode.getChildren().length - 1; i >= children.length; i--) {
          ...
          this.__P_174_11(treeFolder, rootNode);
        }
      },
       ...
       */
      __P_174_5: function __P_174_5(tree) {
        if (tree == null) {
          tree = this.getTarget();
        ...
        if (root != null) {
          ...
          this.__P_174_12(root);

          var model = root.getModel();

          if (model) {
            this.__P_174_13(model);
          }
       ...
       */
      __P_174_12: function __P_174_12(node) {
        var children = node.getChildren() || []; // remove all subchildren

        for (var i = children.length - 1; i >= 0; i--) {
          if (children[i].getChildren().length > 0) {
            this.__P_174_12(children[i]);
          }

          this.__P_174_11(children[i], node);
        }
      },
       ...
       *   treeFolder.
       */
      __P_174_11: function __P_174_11(treeFolder, rootNode) {
        // get the model
        var model = treeFolder.getModel();
        var childPath = this.__P_174_3 || this.getChildPath();
        var childrenGetterName = "get" + qx.lang.String.firstUp(childPath); // if the model does have a child path
        ...
        if (model[childrenGetterName] != undefined) {
          ...
          var oldRef = this.__P_174_2[children.toHashCode()];

          children.removeListenerById(oldRef.changeListenerId);
          model.removeListenerById(oldRef.changeChildernListenerId); // also remove all its children [BUG #4296]

          this.__P_174_12(treeFolder); // delete the model reference


          delete this.__P_174_2[children.toHashCode()];
        } // get the binding and remove it


        this.__P_174_13(model); // remove the folder from the tree


        rootNode.remove(treeFolder); // get rid of the old tree folder
      ...
      */
       ...
       */
      bindProperty: function bindProperty(sourcePath, targetPath, options, targetWidget, modelNode) {
        // set up the binding
        var id = modelNode.bind(sourcePath, targetWidget, targetPath, options); // check for the storage for the references

        if (this.__P_174_0[targetPath] == null) {
          this.__P_174_0[targetPath] = {};
        } // store the binding reference


        var storage = this.__P_174_0[targetPath];
        ...
        if (!this.__P_174_1.includes(targetPath)) {
          this.__P_174_1.push(targetPath);
        }
      },
       ...
       * @param modelNode {var} The model node which should be bound to the target.
       */
      bindPropertyReverse: function bindPropertyReverse(targetPath, sourcePath, options, sourceWidget, modelNode) {
        // set up the binding
        var id = sourceWidget.bind(sourcePath, modelNode, targetPath, options); // check for the storage for the references

        if (this.__P_174_0[sourcePath] == null) {
          this.__P_174_0[sourcePath] = {};
        } // check if there is already a stored item


        var storage = this.__P_174_0[sourcePath];
        qx.core.ObjectRegistry.register(modelNode);
        ...
        if (!this.__P_174_1.includes(sourcePath)) {
          this.__P_174_1.push(sourcePath);
        }
      },
       ...
       */
      bindDefaultProperties: function bindDefaultProperties(treeNode, modelNode) {
        // label binding
        this.bindProperty(this.getLabelPath(), "label", this.getLabelOptions(), treeNode, modelNode); // icon binding
       ...
       * paths.
       */
      __P_174_4: function __P_174_4() {
        // get the first bound property
        var firstProp;

        for (var key in this.__P_174_0) {
          firstProp = key;
          break;
        ...
        for (var hash in this.__P_174_0[firstProp]) {
          // get the data
          var treeNode = this.__P_174_0[firstProp][hash].treeNode;
          var modelNode = qx.core.ObjectRegistry.fromHashCode(hash); // remove the old bindings

          this.__P_174_13(modelNode); // add the new bindings


          this.__P_174_10(modelNode, treeNode);
        }
      },
       ...
       *   to the model node.
       */
      __P_174_10: function __P_174_10(modelNode, treeNode) {
        var delegate = this.getDelegate(); // if a delegate for creating the binding is given, use it

        if (delegate != null && delegate.bindItem != null) {
          delegate.bindItem(this, treeNode, modelNode); // otherwise, try to bind the listItem by default
        } else {
       ...
       *   should be removed.
       */
      __P_174_13: function __P_174_13(modelNode) {
        for (var i = 0; i < this.__P_174_1.length; i++) {
          var property = this.__P_174_1[i];

          var bindingsMap = this.__P_174_0[property][modelNode.toHashCode()];
          ...
          if (bindingsMap != null) {
            ...
            delete this.__P_174_0[property][modelNode.toHashCode()];
          }
        }
      },
    ...
    destruct: function destruct() {
      ...
      this.__P_174_0 = this.__P_174_2 = this.__P_174_1 = null;
    }
  });
  qx.data.controller.Tree.$$dbClassInfo = $$dbClassInfo;
})();

//# sourceMappingURL=Tree.js.map?dt=1664557340465