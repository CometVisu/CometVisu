    ...
    statics: {
      ...
      ready: function ready(callback) {
        ...
        var onWindowLoad = function onWindowLoad() {
          qx.module.Event.__P_298_2 = true;
          callback();
        };
      ...
      $registerEventNormalization: function $registerEventNormalization(types, normalizer) {
        ...
        var registry = qx.module.Event.__P_298_0;
      ...
      $getEventNormalizationRegistry: function $getEventNormalizationRegistry() {
        return qx.module.Event.__P_298_0;
      },
      /**
      ...
      $registerEventHook: function $registerEventHook(types, registerHook, unregisterHook) {
        ...
        var onHooks = qx.module.Event.__P_298_1.on;
        for (var i = 0, l = types.length; i < l; i++) {
          var type = types[i];
        ...
        var offHooks = qx.module.Event.__P_298_1.off;
        for (var i = 0, l = types.length; i < l; i++) {
          var type = types[i];
      ...
      $getEventHookRegistry: function $getEventHookRegistry() {
        return qx.module.Event.__P_298_1;
      }
    },
    members: {
      /**
      ...
      on: function on(type, listener, context, useCapture) {
        for (var i = 0; i < this.length; i++) {
          ...
          // call hooks
          var hooks = qx.module.Event.__P_298_1.on;
          // generic
          var typeHooks = hooks["*"] || [];
          // type specific
          if (hooks[type]) {
          ...
          if (!el.__P_298_3[type]) {
            el.__P_298_3[type] = {};
          }
          el.__P_298_3[type][el.$$lastlistenerId] = bound;
          if (!context) {
            ...
            el.__P_298_4[el.$$lastlistenerId] = ctx;
          }
        }
        return this;
      },
      ...
      off: function off(type, listener, context, useCapture) {
        ...
        for (var j = 0; j < this.length; j++) {
          ...
          if (type !== null) {
            ...
            for (var listenerType in el.__P_298_3) {
              types.push(listenerType);
            }
          }
          for (var i = 0, l = types.length; i < l; i++) {
            for (var id in el.__P_298_3[types[i]]) {
              var storedListener = el.__P_298_3[types[i]][id];
              if (removeAll || storedListener == listener || storedListener.original == listener) {
                // get the stored context
                var hasStoredContext = typeof el.__P_298_4 !== "undefined" && el.__P_298_4[id];
                var storedContext;
                if (!context && hasStoredContext) {
                  storedContext = el.__P_298_4[id];
                }
                ...
                if (result !== null) {
                  delete el.__P_298_3[types[i]][id];
                }
                if (hasStoredContext) {
                  delete el.__P_298_4[id];
                }
              }
            }

            // call hooks
            var hooks = qx.module.Event.__P_298_1.off;
            // generic
            var typeHooks = hooks["*"] || [];
            // type specific
            if (hooks[type]) {
...
//# sourceMappingURL=Event.js.map?dt=1782705780909