  ...
  qx.Class.define("qx.util.format.DateFormat", {
    ...
    */
    ...
    construct: function construct(format, locale) {
      ...
      this.__P_759_0 = this.__P_759_1 = locale;
      if (format != null) {
        this.__P_759_2 = format.toString();
        if (this.__P_759_2 in qx.util.format.DateFormat.ISO_MASKS) {
          if (this.__P_759_2 === "isoUtcDateTime") {
            this.__P_759_3 = true;
          }
          this.__P_759_2 = qx.util.format.DateFormat.ISO_MASKS[this.__P_759_2];
        }
      } else {
        this.__P_759_2 = qx.locale.Date.getDateFormat("long", this.getLocale()) + " " + qx.locale.Date.getDateTimeFormat("HHmmss", "HH:mm:ss", this.getLocale());
      }
    },
    /*
    *****************************************************************************
    ...
    */
    ...
    statics: {
      ...
      getDateInstance: function getDateInstance() {
        ...
        if (DateFormat._dateInstance == null || DateFormat._dateInstance.__P_759_2 != format) {
      ...
      getDateTimeInstance: function getDateTimeInstance() {
        ...
        // Memoizing the instance, so caller doesn't have to dispose it.
        if (DateFormat._dateTimeInstance == null || DateFormat._dateTimeInstance.__P_759_2 != format) {
    ...
    */
    ...
    members: {
       ...
       */
      __P_759_7: function __P_759_7(number, minSize) {
       ...
       */
      __P_759_8: function __P_759_8(date) {
       ...
       */
      __P_759_9: function __P_759_9(date) {
       ...
       */
      __P_759_10: function __P_759_10(date) {
        ...
        var thursdayDate = this.__P_759_9(date);
        ...
        var thursdayWeek1 = this.__P_759_9(new Date(weekYear, 0, 4));
       ...
       */
      __P_759_11: function __P_759_11(date) {
        var thursdayDate = this.__P_759_9(date);
        var thursdayWeek1 = this.__P_759_9(new Date(date.getFullYear(), date.getMonth(), 4));
        return Math.floor(1.5 + (thursdayDate.getTime() - thursdayWeek1.getTime()) / 86400000 / 7);
      },
      /**
       ...
       */
      __P_759_12: function __P_759_12(date) {
        var thursdayDate = this.__P_759_9(date);
       ...
       */
      __P_759_13: function __P_759_13(year) {
       ...
       */
      __P_759_14: function __P_759_14(dayOfYear, year) {
        var month = 0;
        var day = 0;
       ...
       */
      __P_759_15: function __P_759_15(weekYear, month, dayOfMonth) {
        var year;
        switch (month) {
          case 11:
            year = weekYear - 1;
            if (weekYear != this.__P_759_12(new Date(year, month, dayOfMonth))) {
              year = weekYear;
            }
            break;
          case 0:
      ...
      setLocale: function setLocale(value) {
        ...
        this.__P_759_1 = value === null ? this.__P_759_0 : value;
      },
      /**
       * Resets the Locale
       */
      ...
      format: function format(date) {
        ...
        this.__P_759_16();
        var output = "";
        for (var i = 0; i < this.__P_759_6.length; i++) {
          var currAtom = this.__P_759_6[i];
          if (currAtom.type == "literal") {
            ...
            switch (wildcardChar) {
              case "y":
                // Year
                if (wildcardSize == 2) {
                  replacement = this.__P_759_7(fullYear % 100, 2);
                } else {
              ...
              case "Y":
                // Year
                replacement = this.__P_759_12(date) + "";
              ...
              case "Q":
                ...
                if (wildcardSize == 1 || wildcardSize == 2) {
                  replacement = this.__P_759_7(parseInt(month / 4) + 1, wildcardSize);
                }
                if (wildcardSize == 3) {
              ...
              case "D":
                // Day in year (e.g. 189)
                replacement = this.__P_759_7(this.__P_759_8(date), wildcardSize);
                break;
              case "d":
                // Day in month
                replacement = this.__P_759_7(dayOfMonth, wildcardSize);
                break;
              case "w":
                // Week in year (e.g. 27)
                replacement = this.__P_759_7(this.__P_759_10(date), wildcardSize);
                break;
              case "W":
                // Week in year (e.g. 27)
                replacement = this.__P_759_11(date);
                break;
              case "E":
                // Day in week
                if (wildcardSize >= 1 && wildcardSize <= 3) {
              ...
              case "e":
                ...
                if (wildcardSize >= 1 && wildcardSize <= 2) {
                  replacement = this.__P_759_7(localeDayOfWeek, wildcardSize);
              ...
              case "M":
                // Month
                if (wildcardSize == 1 || wildcardSize == 2) {
                  replacement = this.__P_759_7(month + 1, wildcardSize);
                } else if (wildcardSize == 3) {
              ...
              case "H":
                // Hour in day (0-23)
                replacement = this.__P_759_7(hours, wildcardSize);
                break;
              case "k":
                // Hour in day (1-24)
                replacement = this.__P_759_7(hours == 0 ? 24 : hours, wildcardSize);
                break;
              case "K":
                // Hour in am/pm (0-11)
                replacement = this.__P_759_7(hours % 12, wildcardSize);
                break;
              case "h":
                // Hour in am/pm (1-12)
                replacement = this.__P_759_7(hours % 12 == 0 ? 12 : hours % 12, wildcardSize);
                break;
              case "m":
                // Minute in hour
                replacement = this.__P_759_7(minutes, wildcardSize);
                break;
              case "s":
                // Second in minute
                replacement = this.__P_759_7(seconds, wildcardSize);
                break;
              case "S":
                // Fractional second
                replacement = this.__P_759_7(ms, 3);
                if (wildcardSize < replacement.length) {
              ...
              case "z":
                ...
                if (wildcardSize >= 1 && wildcardSize <= 4) {
                  replacement = "GMT" + (timezoneSign > 0 ? "-" : "+") + this.__P_759_7(Math.abs(timezoneHours), 2) + ":" + this.__P_759_7(timezoneMinutes, 2);
                }
                break;
              case "Z":
                // RFC 822 time zone
                if (wildcardSize >= 1 && wildcardSize <= 3) {
                  replacement = (timezoneSign > 0 ? "-" : "+") + this.__P_759_7(Math.abs(timezoneHours), 2) + this.__P_759_7(timezoneMinutes, 2);
                } else {
                  replacement = "GMT" + (timezoneSign > 0 ? "-" : "+") + this.__P_759_7(Math.abs(timezoneHours), 2) + ":" + this.__P_759_7(timezoneMinutes, 2);
                }
                break;
      ...
      parse: function parse(dateStr) {
        ...
        // Apply the regex
        var hit = this.__P_759_4.regex.exec(dateStr);
        if (hit == null) {
          throw new Error("Date string '" + dateStr + "' does not match the date format: " + this.__P_759_2);
        }
        ...
        for (var i = 0; i < this.__P_759_4.usedRules.length; i++) {
          var rule = this.__P_759_4.usedRules[i];
          ...
          if (rule.pattern == "Y+") {
            ...
            for (var k = 0; k < this.__P_759_4.usedRules.length; k++) {
              if (this.__P_759_4.usedRules[k].pattern == "y+") {
          ...
          if (rule.pattern.indexOf("D") != -1) {
            ...
            for (var k = 0; k < this.__P_759_4.usedRules.length; k++) {
              if (this.__P_759_4.usedRules[k].pattern.indexOf("d") != -1) {
                dayRuleApplied = true;
                break;
              }
            }
        ...
        if (applyWeekYearAfterRule) {
          dateValues.year = this.__P_759_15(dateValues.weekYear, dateValues.month, dateValues.day);
        }
        if (applyDayOfYearAfterRule) {
          var dayAndMonth = this.__P_759_14(dateValues.dayOfYear, dateValues.year);
        ...
        var date;
        if (this.__P_759_3 || dateValues.tzOffsetMins !== null) {
          ...
          date = new Date(utcMs);
          if (this.__P_759_3 && (dateValues.month !== date.getUTCMonth() || dateValues.year !== date.getUTCFullYear())) {
       ...
       *
       */
      __P_759_16: function __P_759_16() {
        if (this.__P_759_6 != null) {
          return;
        }
        this.__P_759_6 = [];
       ...
       */
      __P_759_17: function __P_759_17() {
        if (this.__P_759_4 != null) {
        ...
        var usedRules = [];
        var pattern = "^";
        for (var atomIdx = 0; atomIdx < this.__P_759_6.length; atomIdx++) {
          var currAtom = this.__P_759_6[atomIdx];
          if (currAtom.type == "literal") {
            ...
            var wildcardRule;
            for (var ruleIdx = 0; ruleIdx < this.__P_759_5.length; ruleIdx++) {
              var rule = this.__P_759_5[ruleIdx];
              if (this.__P_759_19(rule, wildcardChar, wildcardSize)) {
       ...
       */
      __P_759_19: function __P_759_19(rule, wildcardChar, wildcardSize) {
       ...
       *
       */
      __P_759_18: function __P_759_18() {
        var DateFormat = qx.util.format.DateFormat;
        var LString = qx.lang.String;
        if (this.__P_759_5 != null) {
          // The parse rules are already initialized
          return;
        }
        var rules = this.__P_759_5 = [];
...
//# sourceMappingURL=DateFormat.js.map?dt=1782705805076