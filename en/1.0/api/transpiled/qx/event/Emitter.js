  ...
  qx.Bootstrap.define("qx.event.Emitter", {
    ...
    members: {
      ...
      on: function on(name, listener, ctx) {
        var id = qx.event.Emitter.__P_215_0.length;
        this.__P_215_3(name).push({
          listener: listener,
          ctx: ctx,
          id: id,
          name: name
        });
        qx.event.Emitter.__P_215_0.push({
          name: name,
          listener: listener,
          ctx: ctx
        });
      ...
      off: function off(name, listener, ctx) {
        var storage = this.__P_215_3(name);
        for (var i = storage.length - 1; i >= 0; i--) {
          var entry = storage[i];
          if (entry.listener == listener && entry.ctx == ctx) {
            storage.splice(i, 1);
            qx.event.Emitter.__P_215_0[entry.id] = null;
            return entry.id;
          }
        }
        return null;
      ...
      offById: function offById(id) {
        var entry = qx.event.Emitter.__P_215_0[id];
        if (entry) {
      ...
      emit: function emit(name, data) {
        var storage = this.__P_215_3(name).concat();
        var toDelete = [];
        ...
        toDelete.forEach(function (entry) {
          var origStorage = this.__P_215_3(name);
          var idx = origStorage.indexOf(entry);
          origStorage.splice(idx, 1);
        }.bind(this));

        // call on any
        storage = this.__P_215_3("*");
        for (var i = storage.length - 1; i >= 0; i--) {
          var entry = storage[i];
          entry.listener.call(entry.ctx, data);
        }
      ...
      getEntryById: function getEntryById(id) {
        for (var name in this.__P_215_1) {
          var store = this.__P_215_1[name];
       ...
       */
      __P_215_3: function __P_215_3(name) {
        if (this.__P_215_1 == null) {
          this.__P_215_1 = {};
        }
        if (this.__P_215_1[name] == null) {
          this.__P_215_1[name] = [];
        }
        return this.__P_215_1[name];
      }
    }
  });
...
//# sourceMappingURL=Emitter.js.map?dt=1782705777645