  ...
  qx.Class.define('cv.util.ScriptLoader', {
    ...
    */
    construct: function construct() {
      ...
      this.__P_772_0 = new qx.data.Array();
      this.__P_772_1 = new qx.data.Array();
      this.__P_772_2 = new qx.data.Array();
      this.__P_772_3 = [];
    },
    /*
    ***********************************************
    ...
    members: {
      ...
      addStyles: function addStyles(styleArr) {
        ...
        Promise.all(promises).then(function () {
          ...
          _this2.__P_772_6 = true;
          _this2.fireEvent('stylesLoaded');
          if (_this2.getFinished() && _this2.__P_772_4 && _this2.__P_772_5) {
      ...
      markAsLoaded: function markAsLoaded(path) {
        if (!this.__P_772_3.includes(path)) {
          this.debug('marking ' + path + ' as loaded');
          this.__P_772_3.push(path);
        }
      },
      isMarkedAsLoaded: function isMarkedAsLoaded(path) {
        return this.__P_772_3.includes(path);
      },
      addScripts: function addScripts(scriptArr, order) {
        ...
        this.__P_772_4 = true;
        for (; i < l; i++) {
          if (!this.__P_772_3.includes(queue[i])) {
       ...
       */
      __P_772_8: function __P_772_8(script) {
        var _this3 = this;
        var loader = new qx.util.DynamicScriptLoader(script);
        this.__P_772_1.push(loader);
      ...
      _checkQueue: function _checkQueue() {
        var _this4 = this;
        if (this.__P_772_0.length === 0) {
          if (this.isAllQueued()) {
            this.debug('script loader finished');
            this.fireEvent('finished');
            this.setFinished(true);
            if (this.__P_772_6 && this.__P_772_4 && this.__P_772_5) {
              this.fireEvent('stylesAndScriptsLoaded');
            }
          } else if (!this.__P_772_7) {
            this.debug('script loader waiting for all scripts beeing queued');
            this.__P_772_7 = this.addListener('changeAllQueued', function (ev) {
              if (ev.getData() === true) {
                if (_this4.__P_772_0.length === 0) {
                  _this4.debug('script loader finished');
                  _this4.fireEvent('finished');
                  _this4.setFinished(true);
                }
                _this4.removeListenerById(_this4.__P_772_7);
                _this4.__P_772_7 = null;
              }
            });
          }
        } else {
          this.debug(this.__P_772_0.length + ' scripts remaining');
        }
      }
    }
  });
...
//# sourceMappingURL=ScriptLoader.js.map?dt=1782705805697