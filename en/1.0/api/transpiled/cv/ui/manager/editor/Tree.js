          ...
          return _regenerator().w(function (_context) {
            while (1) switch (_context.n) {
              case 0:
                ...
                if (Object.prototype.hasOwnProperty.call(_this2._schemas, file)) {
                  _context.n = 2;
                  break;
                }
                _context.n = 1;
              ...
              case 1:
                _this2._schemas[file] = _context.v;
              case 2:
        ...
        if (value && file) {
          if (this._workerWrapper) {
            ...
            this._workerWrapper.validateXmlConfig(value).then(function (res) {
              if (res === true) {
                _this0.info(file.getPath() + ' is a valid config file');
                _this0.__P_36_11(value);
              } else {
                var dialog = new cv.ui.manager.dialog.ValidationError(file, value, res);
                dialog.addListener('action', function (ev) {
                  switch (ev.getData()) {
                    case 'proceed':
                      _this0.__P_36_11(value, res);
                      break;
                    case 'open-source':
                      {
                        var _file = _this0.getFile();
                    ...
                    case 'cancel':
                      // close this editor
                      cv.ui.manager.Main.getInstance().closeFile(_this0.getFile());
                      break;
                  }
                  dialog.hide();
                  dialog.destroy();
        ...
        return _asyncToGenerator(/*#__PURE__*/_regenerator().m(function _callee2() {
          ...
          return _regenerator().w(function (_context2) {
            while (1) switch (_context2.n) {
              case 0:
                tree = _this1.getChildControl('tree');
                file = _this1.getFile();
                if (!file) {
                  _context2.n = 2;
                  break;
                ...
                _context2.n = 1;
                return _this1.getSchema(rootElement.getAttributeNS('http://www.w3.org/2001/XMLSchema-instance', 'noNamespaceSchemaLocation'));
              case 1:
                schema = _context2.v;
                schemaElement = schema.getElementNode(rootElement.nodeName);
                rootNode = new cv.ui.manager.model.XmlElement(rootElement, schemaElement, _this1);
                _this1._structure = schema.getStructure();
                rootNode.setEditable(file.getWriteable());
                rootNode.load();
                tree.setModel(rootNode);
                if (_this1.hasChildControl('add-button')) {
                  _this1.getChildControl('add-button').setVisibility(file.getWriteable() ? 'visible' : 'excluded');

                  // extra space für add-button
                  tree.setContentPaddingBottom(file.getWriteable() ? 80 : 0);
                }
                if (_this1.isShowPreview()) {
                  preview = _this1.getChildControl('preview');
                  if (file.isWriteable()) {
                    if (!preview.getFile()) {
                      preview.setFile(_this1.__P_36_6());
                    }
                  } else {
                    preview.setFile(file);
                  }
                  _this1._updatePreview(null, value);
                  if (!preview.isVisible()) {
                    preview.show();
                  }
                }
                if (file.isTemporary()) {
                  _this1._onContentChanged();
                }
                if (errors) {
                        ...
                        if (match) {
                          ...
                          if (current) {
                            try {
                              ...
                              _this1.error('Error loading ' + current.getName() + ': ' + e.toString());
                              current = null;
                              break;
                            }
                          } else {
                          ...
                          _this1.error('patch segment format error: ' + part);
                          current = null;
                          break;
                        }
                      }
          ...
          if (previewFile.isTemporary()) {
            this._client.createSync({
              ...
              if (err) {
                ...
                _this10.setShowPreview(false);
                _this10.error(err);
                cv.ui.manager.snackbar.Controller.error(_this10.tr('Disabling preview because the preview file could not be created.'));
              } else {
                ...
                _this10.__P_36_1.removeAll();
                _this10.resetPreviewState();
...
//# sourceMappingURL=Tree.js.map?dt=1782705767637