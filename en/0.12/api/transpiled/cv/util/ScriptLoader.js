  ...
  qx.Class.define('cv.util.ScriptLoader', {
    ...
    */
    construct: function construct() {
      ...
      this.__P_483_1 = new qx.data.Array();
      this.__P_483_2 = new qx.data.Array();
      this.__P_483_3 = [];
    },
    ...
    */
    members: {
      ...
      markAsLoaded: function markAsLoaded(path) {
        if (!this.__P_483_3.includes(path)) {
          this.debug('marking ' + path + ' as loaded');

          this.__P_483_3.push(path);
        }
      },
      isMarkedAsLoaded: function isMarkedAsLoaded(path) {
        return this.__P_483_3.includes(path);
      },
      addScripts: function addScripts(scriptArr, order) {
        ...
        for (var i = 0, l = queue.length; i < l; i++) {
          if (!this.__P_483_3.includes(queue[i])) {
        ...
        this.__P_483_0.append(realQueue);
        ...
        if (order) {
          var processQueue = function () {
            if (order.length > 0) {
              ...
              var loader = this.__P_483_5(script);

              loader.addListener("ready", processQueue, this);
            } else {
              realQueue.forEach(this.__P_483_5, this);
            }
          }.bind(this);
       ...
       */
      __P_483_5: function __P_483_5(script) {
        var loader = new qx.util.DynamicScriptLoader(script);

        this.__P_483_1.push(loader);
      ...
      _onLoaded: function _onLoaded(ev) {
        ...
        this.__P_483_0.remove(data.script);
      ...
      _checkQueue: function _checkQueue() {
        if (this.__P_483_0.length === 0) {
          ...
          } else if (!this.__P_483_4) {
            this.debug("script loader waiting for all scripts beeing queued");
            this.__P_483_4 = this.addListener("changeAllQueued", function (ev) {
              if (ev.getData() === true) {
                if (this.__P_483_0.length === 0) {
                  this.debug("script loader finished");
                  this.fireEvent("finished");
                ...
                this.removeListenerById(this.__P_483_4);
                this.__P_483_4 = null;
              }
            }, this);
          }
        } else {
          this.debug(this.__P_483_0.length + " scripts remaining");
        }
      }
    }
  });
...
//# sourceMappingURL=ScriptLoader.js.map?dt=1613908130907