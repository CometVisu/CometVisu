    ...
    members: {
      ...
      upgrade: function upgrade(source) {
        ...
        if (version === cv.Version.LIBRARY_VERSION) {
          // nothing to do
          return [null, source, this.__P_495_1];
        } else {
          while (version < cv.Version.LIBRARY_VERSION) {
            ...
            if (method) {
              ...
              return [qx.locale.Manager.tr('Upgrader from version %1 not implemented', version), source, this.__P_495_1];
            }
          }

          this.info('  - ' + this.__P_495_1.join('\n  - '));
          return [null, source, this.__P_495_1];
        }
      },
      from7to8: function from7to8(source) {
        ...
        this.__P_495_2(source, 8);

        if (c > 0) {
          this.__P_495_1.push('removed ' + c + ' \'plugin\'-nodes with obsolete plugin (gweather)');
        }
      ...
      from8to9: function from8to9(source) {
        ...
        var c = 0;
        var singleIndent = ''.padEnd(this.__P_495_0, ' ');
        source.querySelectorAll('multitrigger').forEach(function (node) {
          var level = _this.__P_495_3(node);

          level++;
          var indent = ''.padEnd(_this.__P_495_0 * level, ' ');
          var buttonConf = {};
          ...
          if (buttonIds.length > 0) {
            ...
            node.appendChild(source.createTextNode("\n" + ''.padEnd(_this.__P_495_0 * (level - 1), ' ')));
            c++;
          }
        });

        this.__P_495_2(source, 9);

        if (c > 0) {
          this.__P_495_1.push('converted ' + c + ' \'multitrigger\'-nodes to new button configuration');
        }

        return 9;
      },
      __P_495_3: function __P_495_3(node) {
        var level = 1;
        var parent = node.parentNode;
      ...
      __P_495_2: function __P_495_2(xml, version) {
...
//# sourceMappingURL=ConfigUpgrader.js.map?dt=1612690421598