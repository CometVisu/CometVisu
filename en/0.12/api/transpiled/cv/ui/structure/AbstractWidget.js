    ...
    */
    members: {
      ...
      _onPointerDown: function _onPointerDown(ev) {
        ...
        this.__P_488_0 = ev.getCurrentTarget();
        this.__P_488_1 = Date.now();
        ...
        if (this._onLongTap && qx.Class.hasMixin(this.constructor, cv.ui.common.HandleLongpress) && !this.isSendLongOnRelease() && this.getShortThreshold() > 0) {
          ...
          this.__P_488_2 = qx.event.Timer.once(function () {
      ...
      _onPointerMove: function _onPointerMove(ev) {
        ...
        var distance = Math.max(Math.abs(this.__P_488_3.x - ev.getDocumentLeft()), Math.abs(this.__P_488_3.y - ev.getDocumentTop()));
        var abort = distance > 5;

        if (!abort) {
          while (upElement && upElement !== this.__P_488_0) {
          ...
          abort = !upElement || upElement !== this.__P_488_0;
        }
      ...
      _onPointerUp: function _onPointerUp(ev) {
        ...
        if (upElement && upElement === this.__P_488_0) {
          this._skipNextEvent = "tap"; // both events happened on the same element

          ev.setCurrentTarget(upElement);

          if (this._onLongTap && qx.Class.hasMixin(this.constructor, cv.ui.common.HandleLongpress) && this.getShortThreshold() > 0 && Date.now() - this.__P_488_1 >= this.getShortThreshold()) {
            // this is a longpress
            this._onLongTap(ev);
          } else {
            this.action(ev);
...
//# sourceMappingURL=AbstractWidget.js.map?dt=1613590639393