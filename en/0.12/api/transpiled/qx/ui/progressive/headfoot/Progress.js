  ...
  ************************************************************************ */
  ...
  qx.Class.define("qx.ui.progressive.headfoot.Progress", {
    ...
    construct: function construct(columnWidths, labelArr) {
      ...
      this.set({
        backgroundColor: this.__P_371_0.background
      }); // Create a widget that continually increases its width for progress bar

      this.__P_371_2 = new qx.ui.core.Widget();

      this.__P_371_2.set({
        width: 0,
        backgroundColor: this.__P_371_0.indicatorDone
      });

      this.add(this.__P_371_2); // Create a flex area between the progress bar and the percent done

      var spacer = new qx.ui.core.Widget();
      spacer.set({
        backgroundColor: this.__P_371_0.indicatorUndone
      });
      this.add(spacer, {
        flex: 1
      }); // We also like to show progress as a percentage done string.

      this.__P_371_3 = new qx.ui.basic.Atom("0%");

      this.__P_371_3.set({
        width: 100,
        backgroundColor: this.__P_371_0.percentBackground,
        textColor: this.__P_371_0.percentText
      });

      this.add(this.__P_371_3); // We're initially invisible
    ...
    members: {
       ...
       */
      __P_371_1: function __P_371_1() {
        // link to color theme
        var colorMgr = qx.theme.manager.Color.getInstance();
        this.__P_371_0.background = colorMgr.resolve("progressive-progressbar-background");
        this.__P_371_0.indicatorDone = colorMgr.resolve("progressive-progressbar-indicator-done");
        this.__P_371_0.indicatorUndone = colorMgr.resolve("progressive-progressbar-indicator-undone");
        this.__P_371_0.percentBackground = colorMgr.resolve("progressive-progressbar-percent-background");
        this.__P_371_0.percentText = colorMgr.resolve("progressive-progressbar-percent-text");
      },
      // overridden
      join: function join(progressive) {
        ...
        progressive.addListener("renderStart", function (e) {
          this.__P_371_4 = e.getData().initial;
          this.show();
        }, this); // Listen for the "progress" event, to update the progress bar

        progressive.addListener("progress", function (e) {
          var complete = 1.0 - e.getData().remaining / this.__P_371_4;

          var mySize = this.getBounds();

          if (mySize) {
            var barWidth = Math.floor((mySize.width - this.__P_371_3.getBounds().width) * complete);
            var percent = Math.floor(complete * 100) + "%";

            if (!isNaN(barWidth)) {
              this.__P_371_2.setMinWidth(barWidth);

              this.__P_371_3.setLabel(percent);
            }
          }
    ...
    destruct: function destruct() {
      ...
      this._disposeObjects("__P_371_2", "__P_371_3");
    }
  });
  qx.ui.progressive.headfoot.Progress.$$dbClassInfo = $$dbClassInfo;
})();

//# sourceMappingURL=Progress.js.map?dt=1614551293152