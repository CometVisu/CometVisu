  ...
  ************************************************************************ */
  ...
  qx.Class.define("qx.ui.progressive.Progressive", {
    ...
    construct: function construct(structure) {
      ...
      this.__P_369_0 = {}; // Prepare to have our pane structure added to us.
      ...
      this.__P_369_1 = structure;
      structure.applyStructure(this); // We've not yet done our initial render

      this.__P_369_2 = false; // We're not currently rendering

      this.__P_369_3 = false; // Number of elements available to be rendered.  Useful for progress
    ...
    members: {
      ...
      removeRenderer: function removeRenderer(name) {
        if (!this.__P_369_0[name]) {
          throw new Error("No existing renderer named " + name);
        }

        delete this.__P_369_0[name];
      },
      ...
      render: function render() {
        ...
        var state = new qx.ui.progressive.State({
          ...
          pane: this.__P_369_1.getPane(),
          batchSize: this.getBatchSize(),
          rendererData: this.__P_369_6(),
          userData: {}
        }); // Record render start time

        this.__P_369_5 = new Date(); // Render the first batch of elements.  Subsequent batches will be via
        // timer started from this.__renderElementBatch().

        if (this.__P_369_2) {
          // Get the starting number of elements
          this.__P_369_4 = state.getModel().getElementCount(); // Let listeners know we're beginning to render

          this.fireDataEvent("renderStart", {
            state: state,
            initial: this.__P_369_4
          }); // Begin rendering

          this.__P_369_7(state);
        } else {
          ...
          qx.event.Timer.once(function () {
            this.__P_369_4 = state.getModel().getElementCount();
            this.fireDataEvent("renderStart", {
              state: state,
              initial: this.__P_369_4
            });
      ...
      _applyDataModel: function _applyDataModel(value, old) {
        if (old) {
          ...
          old.removeListener("dataAvailable", this.__P_369_8, this); // Dispose the old model

          old.dispose();
        } // Add an event listener so we know when data is available in the model


        value.addListener("dataAvailable", this.__P_369_8, this);
      },
       ...
       *
       */
      __P_369_7: function __P_369_7(state) {
        var current;
        var element;
        var renderer;
        ...
        for (var i = state.getBatchSize(); i > 0; i--) {
          ...
          if (!current) {
            ...
            this.debug("Render time: " + (new Date() - this.__P_369_5) + "ms");
            this.__P_369_3 = false; // Notify any progress handlers that are listening
          ...
          renderer = this.__P_369_0[element.renderer]; // Render this element
       ...
       */
      __P_369_6: function __P_369_6() {
        var rendererData = {};

        for (var name in this.__P_369_0) {
          rendererData[name] = {};
        }
       ...
       *
       */
      __P_369_8: function __P_369_8(e) {
        this.__P_369_4 = e.getData();
        this.render();
      }
    },
    ...
    destruct: function destruct() {
      ...
      for (var name in this.__P_369_0) {
        // ... dispose it
        this.__P_369_0[name].dispose();
      } // Clean up references


      this.__P_369_5 = this.__P_369_0 = this.__P_369_1 = null;
    }
  });
  qx.ui.progressive.Progressive.$$dbClassInfo = $$dbClassInfo;
})();

//# sourceMappingURL=Progressive.js.map?dt=1619360996982