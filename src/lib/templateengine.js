    ...
    (function (outerThis) { // closure to keep namespace clean
      ...
      window.addEventListener(isTouchDevice ? 'touchend' : 'mouseup', function (event) {
        if (isWidget) {
          ...
          if (
            actionFn !== undefined &&
            inCurrent && !mouseEvent.alreadyCanceled
          ) {
      ...
      !isTouchDevice && window.addEventListener('mousemove', function (event) {
        if (isWidget) {
          ...
          if (inCurrent && mouseEvent.alreadyCanceled) { // reactivate
            mouseEvent.alreadyCanceled = false;
            var
              actionFn = mouseEvent.widgetCreator.downaction;
            actionFn && actionFn.call(widget, widget.id, mouseEvent.actor, false, event);
          } else if ((!inCurrent && !mouseEvent.alreadyCanceled)) { // cancel
      ...
      isTouchDevice && window.addEventListener('touchmove', function (event) {
        if (isWidget) {
          ...
          if (mouseEvent.moveRestrict && !mouseEvent.alreadyCanceled
            ...
            || (touchStartY + 5 < parseInt(touchobj.clientY) || touchStartY - 5 > parseInt(touchobj.clientY)))) { // cancel
            mouseEvent.alreadyCanceled = true;
            var
              actionFn = mouseEvent.widgetCreator.action;
            actionFn && actionFn.call(widget, widget.id, mouseEvent.actor, true, event);
    ...
    this.handleResize = function (resize, skipScrollFix, force) {
      ...
      if (this.mobileDevice) {
        ...
        if (($('#navbarTop').css('display') != "none" && $('#navbarTop').outerHeight(true) <= 2)
          ...
          setTimeout(function () {
            thisTemplateEngine.handleResize(resize, skipScrollFix, true);
          }, 100);
        }
      }
