...
define( ['icon/iconconfig', 'lib/iconset'], function( iconconfig, iconset ) {
  "use strict";
  ...
  function icon() { // Konstruktor
     ...
     *          iconclass
     */
    this.getIconText = function(name, type, flavour, color, styling, iconclass) {
      var svgIcon = iconset.getInstance().getIcon(name);
      if (svgIcon !== null) {
        var style="";
        var classes="icon";
        var size = "2em";
        ...
        return '<i class="'+classes+'"'+style+'>'+svgIcon+'</i>';
      }
      else {
        var i = this.get.apply(this, arguments);
        if (i) {
