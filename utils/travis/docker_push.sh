...
docker build -f Dockerfile -t "${IMAGE_NAME}:${VERSION_TAG}-arm" --build-arg CONTAINER_FROM="hansmp/somedev:dev" --build-arg BUILD_DATE="$BUILD_DATE" --build-arg VCS_REF="$VCS_REF" --build-arg VERSION_TAG="$VERSION_TAG" --build-arg TRAVIS_JOB_NUMBER --build-arg TRAVIS_JOB_WEB_URL --build-arg TRAVIS_BUILD_WEB_URL .
